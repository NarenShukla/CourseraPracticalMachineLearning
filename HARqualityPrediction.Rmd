...
6. **Boosting :** Statistical Learning method that learns slowly by fitting small trees to the residuals and Applying weights. Does not use Bootstrap Sampling. Each tree depends upon previously grown tree
...
The confusion matrix shows **`r accLdaModel`%** accuracy. Out of Sample error is **`r errLdaModel`%**. So Cross-Validated SingleTree model actually did better. Let us continue exploring other models. 


### Random Forest

Let us apply **10 fold cross-validation** here,
...
Let us plot the **Variable Importance** now,
...
Let us use this model against **validation** data-set,
...
Now let us apply this model to **validation** data-set,
...
Let us plot ROC curves for all models together.
